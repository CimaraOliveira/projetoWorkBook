...
async function atualizarNotificacoes() {
    ...
    if (resp.status === 200) {
        ...
        await renderNotificacao([])
    }
}

async function getByRemetenteAndDestinatario(remetente, destinatario){
    const resp = await Req.getJSON({uri: uri_api_notificacao_get_by_remetente_and_destinatario, params: {remetente, destinatario}})
    if (resp.status === 200){
        ...
        if (data.length === 0){
            ...
            for (let obj of data) {
                ...
                if (resp.status === 200) {
                    ...
                    const a = new Tag({ tagName: 'a', children: [spanApp, divApp], attrs: { class: 'app-notification__item', onclick: `save_id_perfil(${mensagem.remetente}, ${JSON.stringify(notificacao)})`, href: '#' } })
